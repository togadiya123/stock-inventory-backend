...
export const login = async (request, response) => {
    try {
        ...
        response
            .status(400)
            .send({ message: error.message, error: error.message });
    }
};

export const signup = async (request, response) => {
    try {
        const { value, error } = sighupBodySchema.validate(request.body, {
            abortEarly: false,
        });
        if (error) return schemaErrorResponse({ response, error });
...
export const validateToken = async (request, response) => {
    try {
        ...
        response
            .status(400)
            .send({ message: "Authentication required", error: error.message });
    }
};
